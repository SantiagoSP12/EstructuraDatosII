    ...
    public void ejecutarPrim() throws ExcepcionAristaYaExiste {
        ...
        do {
            ...
            for (G vertice : grafoAuxiliar.getVertices()) {
                ...
                List<AdyacenteConPeso> adyacentes = elGrafoPesado.listasDeAdyacencias.get(nroOrigen);
                for (int i = 0; i < adyacentes.size(); i++) {
                    G ady = elGrafoPesado.listaDeVertices.get(adyacentes.get(i).getNroVertice());
                    if (grafoAuxiliar.nroVertice(ady) == GrafoPesado.NRO_DE_VERTICE_INVALIDO) {
                        if (adyacentes.get(i).getPeso() < control.getPeso()) {
                            origen = vertice;
                            control = adyacentes.get(i);
                        }
                    }
                }
            }
